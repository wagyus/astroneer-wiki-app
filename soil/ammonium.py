...
def call_result1(label_result, ammonium, result1):
    ammonium = int((ammonium.get()))
    canister_total = ammonium * 2
...
labelTitle = tk.Label(root, text="Ammonium Soil Calculator").grid(row=0, column=2)
labelammonium= tk.Label(root, text="# of ammonium desired: ").grid(row=1, column=0)
labelResult = tk.Label(root)
labelResult.grid(row=2, column=2)
entryammonium = tk.Entry(root, textvariable=number1).grid(row=1, column=2)
