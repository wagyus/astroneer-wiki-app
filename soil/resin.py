...
def call_result1(label_result, resin, result1):
    resin = int((resin.get()))
    canister_total = resin * 0.25
...
labelTitle = tk.Label(root, text="Resin Soil Calculator").grid(row=0, column=2)
labelresin= tk.Label(root, text="# of resin desired: ").grid(row=1, column=0)
labelResult = tk.Label(root)
labelResult.grid(row=2, column=2)
entryresin = tk.Entry(root, textvariable=number1).grid(row=1, column=2)
