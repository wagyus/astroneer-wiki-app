...
def call_result1(label_result, organic, result1):
    organic = int((organic.get()))
    canister_total = organic * 0.25
...
labelTitle = tk.Label(root, text="Organic Soil Calculator").grid(row=0, column=2)
labelorganic= tk.Label(root, text="# of organic desired: ").grid(row=1, column=0)
labelResult = tk.Label(root)
labelResult.grid(row=2, column=2)
entryorganic = tk.Entry(root, textvariable=number1).grid(row=1, column=2)
