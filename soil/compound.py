...
def call_result1(label_result, compound, result1):
    compound = int((compound.get()))
    canister_total = compound * 0.25
...
labelcompound= tk.Label(root, text="# of compound desired: ").grid(row=1, column=0)
labelResult = tk.Label(root)
labelResult.grid(row=2, column=2)
entrycompound = tk.Entry(root, textvariable=number1).grid(row=1, column=2)
